...
|---|---|---|---|
| CPU reference workload<br><sub>best of 3; 0.97x the fastest of this machine's last 5 runs</sub> | `0.2312 s` | `inf s` | PASS |

**Markovian lift**

| check | measured | threshold | verdict |
|---|---|---|---|
| rough objective evaluation, 10 expiries x 13 strikes<br><sub>best of 3 (all: 0.46, 0.46, 0.46); trend target: rolling mean <= 0.75 s</sub> | `0.4603 s` | `3 s` | PASS |
...
| ETDRK4 vs implicit trapezoidal + Richardson, 800-step reference (H=0.12)<br><sub>independent time-steppers, 1 d / 30 d / 1 y</sub> | `6.16e-07` | `2.00e-06` | PASS |
...
| exact-moment filter: QML kappa bias away from zero, 16-substep data, mean z<br><sub>near zero a Gaussian quasi-likelihood is unreliable (open flag)</sub> | `0.1031 SE` | `2 SE` | PASS |
...
| check | measured | threshold | verdict |
|---|---|---|---|
| hedged Monte Carlo price vs the cf price<br><sub>0.021529 +/- 0.000071 vs 0.021588; a low-variance price estimator</sub> | `0.8201 SE` | `3 SE` | PASS |
| variance swap is a martingale: E[M_T] = M_0<br><sub>M_0 0.003333; it pays the option's own realised variance</sub> | `0.2089 SE` | `3 SE` | PASS |
| residual with the variance swap / with the underlying alone<br><sub>0.104 vs 0.273 of the price; the lifted model is complete with both instruments</sub> | `0.3803` | `0.6` | PASS |
...
| startup to actionable error, no TWS<br><sub>was 9.6 s before lazy imports</sub> | `0.5905 s` | `3 s` | PASS |
| calibration objective evaluation<br><sub>39 quotes; 470 ms before vectorising</sub> | `8.597 ms` | `200 ms` | PASS |
...
124 of 124 checks pass (121s).

Trend over 15 recorded run(s); compared within this machine and configuration (LAPTOP-1VCRA7FA, lift 40:1e+08)
--------------------------------------------------------------------------
  check                                            n       mean        std       last  flags
  2025 reference split: session / overnight / we   6          0          0          0  
  3-point stencil order on the same grid (kept f   6      1.086          0      1.086  
  7-day IV, default lift vs true rough Heston (f   6    0.02961          0    0.02961  
  95% interval coverage, 20 chains with 2% noise   6       0.95   1.22e-16       0.95  
  CPU reference workload                           6      0.339    0.08198     0.2312  
  Carr-Madan insensitive to damping alpha          6   6.11e-16          0   6.11e-16  
  Carr-Madan vs exact Black-76                     6   1.94e-16          0   1.94e-16  
  ETDRK4 vs implicit trapezoidal + Richardson (H   5   4.12e-06          0   4.12e-06  
  ETDRK4 vs implicit trapezoidal + Richardson, 8   1   6.16e-07          0   6.16e-07  
  Fourier vs Monte Carlo (sigmas)                  6     0.3618   6.08e-17     0.3618  
  Gauss-Legendre rules constructed                 6          1          0          1  
  H from ATM skew (planted 0.12)                   6       0.12   1.52e-17       0.12  
  H from structure function, worst of 4 planted    6     0.0062          0     0.0062  
  Heston: property switches on at alpha = kappa    6   1.38e-06          0   1.38e-06  
  Ito isometry of the lift at 1 day, shortfall v   6      5.058          0      5.058  
  Jacobian condition number at the solution        6      30.05   3.89e-15      30.05  
  K(t) = int e^-xt mu(dx) representation           6   6.67e-10          0   6.67e-10  
  Labor Day weekend: Friday close to Tuesday ope   6       89.5          0       89.5  
  Laplace-domain error on the pricing band z in    6   0.006354          0   0.006354  
  Laplace-domain error, z in [0.1, 100] (plan: 1   6    0.03349          0    0.03349  
  Lewis vs Carr-Madan on Heston                    6   4.99e-15          0   4.99e-15  
  Lewis vs exact Black-76                          6   3.55e-15          0   3.55e-15  
  MLE plant-and-recover, worst |z|                 6      1.004   2.43e-16      1.004  
  N=1 at x=0 reproduces closed-form Heston (ETDR   6   7.09e-10   1.13e-25   7.09e-10  
  NYSE 2026 holidays by rule vs the published ca   6          0          0          0  
  QE step: mean vs exact conditional mean, in SE   6   0.009151          0   0.009151  
  RMSE improvement minus steady-state theory       6     0.3392          0     0.3392  
  SE(H), 20 quotes, equal weights, 0.5 vp noise    6    0.04652          0    0.04652  
  SE(H), same quotes, vega^2 weights               6     0.1967   3.04e-17     0.1967  
  SE(H): inverse variance / vega^2                 6     0.1018   1.52e-17     0.1018  
  SVI parameter recovery                           6   1.43e-08          0   1.43e-08  
  SVI slice passes Durrleman                       6          1          0          1  
  US close in UTC follows daylight saving          6          1          0          1  
  adaptive: bad-print excursion (x normal error)   6      14.92   1.95e-15      14.92  
  away from zero, 400 days: both filters' log-li   6       1.91          0       1.91  
  branch continuity, shipped (max/median step)     6      5.242          0      5.242  
  branch discontinuity, reciprocal form            6       2825          0       2825  
  butterfly audit: catches a planted dent          6          2          0          2  
  butterfly audit: false positives on a convex s   6          0          0          0  
  calendar audit: catches a reversal               6          3          0          3  
  calendar audit: clean on monotone w              6          0          0          0  
  calibration objective evaluation                 6      12.82       3.19      8.597  
  corr(H, xi) at the truth                         6     0.9777   1.22e-16     0.9777  
  count variance / Hawkes (1971) closed form, 0.   6      1.019          0      1.019  
  delayed greeks (tick 83) recorded, vega per 1.   6          1          0          1  
  density non-negativity (min/peak)                6  -8.87e-10          0  -8.87e-10  
  discount factor from parity                      6   4.44e-16          0   4.44e-16  
  dual vs joint EKF agreement                      6      0.129   3.04e-17      0.129  
  exact-moment filter: QML kappa bias away from    1     0.1031          0     0.1031  
  exact-moment filter: QML kappa bias away from    5      1.056          0      1.056  
  exptrap at 120 steps, u to 1200, guards off (N   6   1.08e+36          0   1.08e+36  
  fast scalar path vs generic matrix filter        6          0          0          0  
  fd_first exact on a quadratic                    6   7.46e-14          0   7.46e-14  
  fd_second on a quadratic vs its rounding floor   6       0.41   6.08e-17       0.41  
  filter profile likelihood: |H_hat - 0.12| in S   6     0.4698          0     0.4698  
  fitted RMSE on clean data                        6   4.06e-09          0   4.06e-09  
  forward from put-call parity                     6   1.14e-13          0   1.14e-13  
  full N-node lift at H = 0.4999 vs Heston         6   1.65e-05          0   1.65e-05  
  hedged Monte Carlo price vs the cf price         1     0.8201          0     0.8201  
  implied-vol round trip                           6   8.05e-16          0   8.05e-16  
  kernel error of the default lift, t in [1 day,   6   0.007027          0   0.007027  
  kernel error of the default lift, worst over H   6   0.009075          0   0.009075  
  kernel error of the default lift, worst over H   6   0.009075          0   0.009075  
  kernel error on [1 hour, 2 y]                    6   0.007027          0   0.007027  
  kernel error on [5 minutes, 2 y]                 6   0.007027          0   0.007027  
  kurtosis vs scale-mixture closed form, in SE     6     0.7597          0     0.7597  
  lecture AR(1) kappa on noisy quotes / true kap   6       12.6          0       12.6  
  lifted path vs true-kernel Volterra path (L2,    6   0.006333          0   0.006333  
  lifted recursion == SOE convolution (identity)   6   3.39e-14          0   3.39e-14  
  lifted rough filter: RMSE improvement over quo   6      37.67   7.78e-15      37.67  
  log-likelihood of H = 0.49 against the best H    6     -329.7          0     -329.7  
  mean rate vs mu/(1-n), in SE                     6     0.5936          0     0.5936  
  min |corr(H, xi)| over all schemes and designs   6     0.9219          0     0.9219  
  naive-form cancellation at xi=1e-6 (kept to pr   6   3.44e-05          0   3.44e-05  
  near zero (V = 0 on 14% of days): particle log   6       68.4          0       68.4  
  near zero: CF filter vs particle filter log-li   6      1.174          0      1.174  
  norm_cdf vs scipy                                6   2.22e-16          0   2.22e-16  
  norm_pdf vs scipy                                6          0          0          0  
  phi(-i) = 1 (martingale)                         6          0          0          0  
  phi(-u) = conj(phi(u))                           6          0          0          0  
  phi(0) = 1                                       6          0          0          0  
  plant-and-recover, worst relative error          6   3.13e-11          0   3.13e-11  
  planted omega = 0.2 recovered from a noise-fre   6          0          0          0  
  put-call parity C-P = F-K                        6   2.84e-14          0   2.84e-14  
  quadrature of the integrated-variance moments    6   1.11e-15          0   1.11e-15  
  quadrature tolerance 1e-12 vs 1e-14              6          0          0          0  
  recursive MLE (Ljung) distance from the MLE      6     0.2287          0     0.2287  
  replay round-trip fidelity                       6          0          0          0  
  residual with the variance swap / with the und   1     0.3803          0     0.3803  
  risk-neutral density mass                        6          1          0          1  
  risk-neutral density mean = forward              6          1          0          1  
  robust: steps to re-converge (and ignores the    6        1.5          0        1.5  
  rough (driver jumps): pairs with the property    6          0          0          0  
  rough jump: realised 1-day impact / its parame   6   0.005225          0   0.005225  
  rough objective evaluation, 10 expiries x 13 s   6     0.6729     0.1649     0.4603  
  rough variance simulated 3000 days: minimum      6  -1.39e-17          0  -1.39e-17  
  runaway parameters: maturities priced in-band    6          4          0          4  
  sampling density: min >= eps and mass = 1        6          0          0          0  
  scipy modules on the live import path            6          0          0          0  
  second spike: superposition identity, every ho   6   8.88e-16          0   8.88e-16  
  shipped 2nd derivative: order on a $1/$5 kink    6      3.077          0      3.077  
  simulated E[S/F] = 1                             6   1.94e-06          0   1.94e-06  
  snapshot instant recovered from the taus (loca   6          0          0          0  
  spread of kappa under 0.5vp noise                6      34.73   7.78e-15      34.73  
  spread of kappa with a Tikhonov prior (weight    6    0.07281          0    0.07281  
  spread of rho under 0.5vp noise                  6       3.73          0       3.73  
  spread of theta under 0.5vp noise                6      2.803   4.86e-16      2.803  
  spread of v0 under 0.5vp noise                   6      3.775          0      3.775  
  spread of xi under 0.5vp noise                   6      19.13          0      19.13  
  stability constants enforced: the same solve r   6          1          0          1  
  stability-sized exptrap, same u range: max |ph   6     0.9988   1.22e-16     0.9988  
  startup to actionable error, no TWS              6      0.745     0.1475     0.5905  
  strict: steps to re-converge after the regime    6         18          0         18  
  tau: 17:00 Istanbul to next close, hours         6         30          0         30  
  time rescaling KS p, Hawkes fit (should pass)    6     0.5838   1.22e-16     0.5838  
  time rescaling KS p, Poisson fit to the same e   6          0          0          0  
  trap / shipped separation                        6      538.9          0      538.9  
  two independent H routes agree                   6   0.002891          0   0.002891  
  unpriceable quote: minimum charge (vol) vs 0 b   6      4.754   9.73e-16      4.754  
  variance cf (Riccati) vs exact affine mean and   6   5.03e-07          0   5.03e-07  
  variance swap is a martingale: E[M_T] = M_0      1     0.2089          0     0.2089  
  variance time at omega = 1 vs ACT/365            6          0          0          0  
  vega vs central difference                       6   4.23e-09          0   4.23e-09  
  worst-case RMS error in H: hybrid (anchors x25   6    0.03783   7.60e-18    0.03783  
  xi -> 0 degenerates to Black-Scholes             6   2.22e-16          0   2.22e-16  
  |phi(u)| <= 1                                    6     0.9999          0     0.9999  
  0 flagged
