...
def h_finite_difference():
    ...
    record("Foundations", "fd_second exact on a quadratic", e2, 1e-10, e2 < 1e-10)
...
def h_bs_degeneracy():
    ...
    q = HestonParams(sigma ** 2, 2.0, sigma ** 2, 1e-4, 0.0)
    e = float(np.max(np.abs(char_func(u, tau, q) - bs_cf(sigma, tau)(u))))
    record("Heston cf", "xi -> 0 degenerates to Black-Scholes", e, 1e-8, e < 1e-8,
           note="at xi=1e-4; 3.5e-06 at xi=1e-6 by cancellation")
...
def h_fractional_kernel():
    ...
    for H in (0.10, 0.12, 0.30):
        ...
        for t in (1 / 365, 0.05, 0.5, 2.0):
            num, _ = quad(lambda x: math.exp(-x * t) * x ** (-a)
                          / (gamma(a) * gamma(1 - a)), 0, np.inf, limit=400)
            exact = t ** (a - 1) / gamma(a)
            worst = max(worst, abs(num - exact) / exact)
    record("Session D readiness", "K(t) = int e^-xt mu(dx) representation", worst,
...
CHECKS = [h_normal, h_black_scholes, h_finite_difference, h_char_func,
          ...
          h_fractional_kernel, h_engineering, h_replay]
