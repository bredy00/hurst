...
## Note added in Session E (2026-09-13): the 58% depends on the weighting

The fit above used vega² weights from a 0.5%-of-forward half-spread. Those
weights leave about two effective quotes on a surface like this one, none of
them short-dated (`study_h_identifiability.py`), so the one-day quotes barely
enter the objective and Heston's one-day skew is left wherever the long end puts
it. Refitted with every quote weighted equally in vol (Session E,
`docs/phase2_rough_vs_heston.md`), vanilla Heston fits this same stylised
surface at 2.10 vol points with κ = 10.9, ξ = 3.80, and its one-day ATM skew
is **201%** of the market's, not 58%.

What survives both weightings is the misfit at the short end, not its sign. The
statement "Heston's short-dated skew is too small" is a property of the
weighting. On a market generated by rough Heston, where the answer is known,
vanilla Heston's log-log skew curve flattens to a slope of −0.12 between one and
three days while the market's is −0.44. That is the structural result.
